...
/* ---------- Config ---------- */
const MIN_TXNS = parseInt(process.env.MIN_TXNS || '3', 10);  // random generator per day (lower bound)
const MAX_TXNS = parseInt(process.env.MAX_TXNS || '5', 10);  // random generator per day (upper bound)
const MANUAL_CAP = parseInt(process.env.MANUAL_CAP || '5', 10); // "Generate Now" daily cap
const TIMEZONE = process.env.TIMEZONE || 'America/Los_Angeles';

/* ---------- Transaction pool: 7 categories x 20 merchants each ---------- */
...
function todayKeyInTZ(tz = TIMEZONE) {
  const now = new Date();
...
function randomAmount(info) {
  ...
  const sign = Math.random() < 0.12 ? -1 : 1; // refunds
  return Number((sign * v).toFixed(2));
}

/* ---------- Transaction generation ---------- */
async function makeTransaction(accountRef) {
...
/* ----------------- Manual Generate Now (v2) -----------------
   GET /tick with Bearer token.
   Creates exactly one transaction if manual cap (5/day) not reached.
   Returns JSON: { ok:true, remaining, item } or 429 + { error:"daily_limit_reached", remaining:0 }
---------------------------------------------------------------- */
...
/* ----------------- API (account + transactions) (v2) ----------------- */
exports.api = onRequest(async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Headers', 'Authorization, Content-Type');
  if (req.method === 'OPTIONS') return res.status(204).end();

  // Normalize path so it works whether the request is `/account` or `/api/account`
...
if (req.method === 'GET' && (normPath === '/transactions' || normPath.endsWith('/transactions'))) {
  ...
  const q = accountRef.collection('transactions').orderBy('timestamp', 'desc').limit(Number(limit));
  ...
  snap.forEach((doc) => {
    ...
    if (!sinceMs || tsVal.getTime() >= sinceMs) {
      txns.push({ ...t, timestamp: tsVal.toISOString() });  // <-- send ISO string
    }
  });
